...
public:
    ...
    PLATON_EVENT2(LogCampaignAdded, u128, platon::Address, uint64_t, u128, uint16_t, uint16_t, platon::u128);
...
public:
    ...
    ACTION u128 NewCampaign(uint64_t burn_num, const u128 &deposit, uint16_t commit_balkline, uint16_t commit_deadline)
    {
        ...
        DEBUG("campaign_id", campaign_id, "form", platon_caller().toString(), "burn_num", burn_num, "deposit", deposit, "commit_balkline", commit_balkline,
        "commit_deadline", commit_deadline, "bountypot", platon_call_value());
    ...
    {
        // check parameter
        if (campaign_id > u128(campaigns.self().size()))
            platon_revert();
        ...
        DEBUG("campaign_id", campaign_id, "from",  platon_caller().toString(), "bountypot", platon_call_value());
    }
    ...
    ACTION void Commit(u128 campaign_id, const bytes32 &hs)
    {
        ...
        DEBUG("campaign_id", campaign_id, "from",  platon_caller().toString(), "hs", toHex(hs));
    }
    ...
    ACTION void Reveal(u128 campaign_id, u128 s)
    {
        ...
        DEBUG("campaign_id", campaign_id, "from",  platon_caller().toString(), "s", std::to_string(s))
    }
    ...
    u128 CalculateShare(const Campaign &cam){
        ...
        if(cam.commit_num > cam.reveals_num) {
            result = u128(cam.commit_num - cam.reveals_num) * cam.deposit / u128(cam.reveals_num);
        }else{
